...
// Count common selected nodes
int similarity_nodes(const vector<int>& A, const vector<int>& B) {
    unordered_set<int> sA(A.begin(), A.end());
    int cnt = 0;
    for (int x : B) if (sA.count(x)) cnt++;
    return cnt;
}

// Count common edges (unordered)
int similarity_edges(const vector<int>& A, const vector<int>& B) {
    int K = A.size();
    unordered_set<long long> edgesA;
    edgesA.reserve(K * 2);

    auto make_key = [&](int x, int y) {
    ...
    for (int i = 0; i < K; ++i) {
        int u = B[i];
        int v = B[(i+1)%K];
        if (edgesA.count(make_key(u, v))) cnt++;
    }
    return cnt;
}
 
void run_convexity_tests(const Instance& I, mt19937& rng, const vector<int>& very_good) {
    ...
    cerr << "\n=== Convexity test for instance " << I.name << " ===\n";

    // ----- 1) Generate 1000 local optima using GREEDY LS -----
    for (int r = 0; r < NUM; ++r) {
        auto start = random_solution(I, rng, -1);
        auto opt = local_search_greedy(start, I, EDGE_EXCHANGE_2OPT, rng);
        locals.push_back(opt);
        objs.push_back(objective(opt, I));

        if ((r+1) % 50 == 0) cerr << r+1 << "/" << NUM << " local optima...\n";
    }

    // Find best of 1000
    int idx_best1000 = min_element(objs.begin(), objs.end()) - objs.begin();
    auto best1000 = locals[idx_best1000];

    int K = I.K;

    // Precompute similarity matrices
    vector<double> avg_nodes(NUM, 0.0), avg_edges(NUM, 0.0);
    vector<int> sim_to_best_nodes(NUM), sim_to_best_edges(NUM);
    vector<int> sim_to_ext_nodes(NUM),  sim_to_ext_edges(NUM);

    // ----- 2) Compute all similarities -----
    for (int i = 0; i < NUM; ++i) {

        // average similarities
        long long sumN = 0, sumE = 0;

        for (int j = 0; j < NUM; ++j) if (i != j) {
            sumN += similarity_nodes(locals[i], locals[j]);
            sumE += similarity_edges(locals[i], locals[j]);
        }

        avg_nodes[i] = sumN / 999.0;
        avg_edges[i] = sumE / 999.0;

        // similarity to best of 1000
        sim_to_best_nodes[i] = (i == idx_best1000 ? -1 : similarity_nodes(locals[i], best1000));
        sim_to_best_edges[i] = (i == idx_best1000 ? -1 : similarity_edges(locals[i], best1000));

        // similarity to external very good solution (ILS best)
        sim_to_ext_nodes[i]  = similarity_nodes(locals[i], very_good);
        sim_to_ext_edges[i]  = similarity_edges(locals[i], very_good);
    }

    // ----- 3) Write CSV files -----
    string base = "convexity_" + I.name + "_";

    auto write_csv = [&](string filename,
                         const vector<double>& sims,
                         const vector<long long>& objs,
                         bool skip_best = false) {

        ofstream F(filename);
        F << "obj,sim\n";
        for (int i = 0; i < NUM; ++i) {
            if (skip_best && sims[i] < 0) continue;
            F << objs[i] << "," << sims[i] << "\n";
        }
        F.close();
    };

    auto write_csv_int = [&](string filename,
                         const vector<int>& sims,
                         const vector<long long>& objs,
                         bool skip_best = false) {
    ...
    // average similarity
    write_csv(base + "avg_nodes.csv", avg_nodes, objs, false);
    write_csv(base + "avg_edges.csv", avg_edges, objs, false);

    // similarity to best of 1000
    write_csv_int(base + "best_nodes.csv", sim_to_best_nodes, objs, true);
    write_csv_int(base + "best_edges.csv", sim_to_best_edges, objs, true);

    // similarity to external very good solution
    write_csv_int(base + "ext_nodes.csv", sim_to_ext_nodes, objs, false);
    write_csv_int(base + "ext_edges.csv", sim_to_ext_edges, objs, false);

    cerr << "Convexity CSV files written.\n";
}
...
for (int i=1; i<argc; ++i) {
    ...
    for (const auto& path : files) {
        ...
        // methods.push_back({"LS_Rand_Steep_Edge_LM", LLONG_MAX, {}, {}, {}}); // 20
        // methods.push_back({"LS_Greedy_Steep_Edge_LM", LLONG_MAX, {}, {}, {}}); // 21

        // // MSLS and ILS
        methods.push_back({"MSLS_LS_Steep_Edge_LM", LLONG_MAX, {}, {}, {}}); // 0
        methods.push_back({"ILS_LS_Steep_Edge_LM",  LLONG_MAX, {}, {}, {}}); // 1
        ...
        //     // LS_Rand_Steep_Edge_LM (20)
        //     current_ls_rand_run++;
        //     print_progress((double)current_ls_rand_run / total_ls_rand_runs,
        //                    "LS from Random (Steep, Edge LM) " + to_string(i + 1) + "/" + to_string(random_tours.size()));
        //     sw.reset();
        //     auto tour20 = local_search_steepest_with_LM(start_tour, I, rng);
        //     long long obj20 = objective(tour20, I);
        //     methods[20].all_objs.push_back(obj20);
        //     methods[20].all_times_ms.push_back(sw.elapsed_ms());
        //     if (obj20 < methods[20].best_obj) { methods[20].best_obj = obj20; methods[20].best_tour = tour20; }
        // }
        // cerr << "\nDone." << endl;
        ...
        // int total_ls_greedy_runs = greedy_tours.size() * 5; // 4 LS variants
        ...
        //     // LS_Greedy_Steep_Edge_LM (21)
        //     current_ls_greedy_run++;
        //     print_progress((double)current_ls_greedy_run / total_ls_greedy_runs,
        //                 "LS from Greedy (Steep, Edge + LM) " + to_string(i + 1) + "/" + to_string(greedy_tours.size()));
        //     sw.reset();
        //     auto tour21 = local_search_steepest_with_LM(start_tour, I, rng);
        //     long long obj21 = objective(tour21, I);
        //     methods[21].all_objs.push_back(obj21);
        //     methods[21].all_times_ms.push_back(sw.elapsed_ms());
        //     if (obj21 < methods[21].best_obj) { methods[21].best_obj = obj21; methods[21].best_tour = tour21; }
        ...
        run_convexity_tests(I, rng, methods[idx_ILS].best_tour);
